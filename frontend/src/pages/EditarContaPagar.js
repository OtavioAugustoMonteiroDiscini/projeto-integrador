...
const EditarContaPagar = () => {
    ...
    () => contaPagarService.buscar(id),
    {
      onSuccess: (data) => {
        if (data) {
          ...
          setValue('dataPagamento', data.dataPagamento ? new Date(data.dataPagamento).toISOString().split('T')[0] : '');
  ...
  const atualizarContaMutation = useMutation(
    (data) => contaPagarService.atualizar(id, data),
    {
      onSuccess: () => {
      ...
      onError: (error) => {
        const errorMessage = error.response?.data?.error || 'Erro ao atualizar conta a pagar';
  ...
  if (!conta) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500">Conta a pagar não encontrada</p>
        <Link to="/contas-pagar" className="btn btn-primary mt-4">
          Voltar
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Editar Conta a Pagar</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="card">
          ...
          <div className="card-body grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label htmlFor="fornecedor" className="label">Fornecedor *</label>
              <input
                type="text"
                id="fornecedor"
                ...
                className="input"
              />
              {errors.fornecedor && <p className="text-danger-600 text-sm mt-1">{errors.fornecedor.message}</p>}
            </div>
            <div>
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                className="input"
                placeholder="Observações da conta a pagar"
              ></textarea>
        ...
        <div className="flex justify-end space-x-4">
          <Link to="/contas-pagar" className="btn btn-secondary">
            Cancelar
          </Link>
          <button
            type="submit"
          ...
          >
            {atualizarContaMutation.isLoading ? <LoadingSpinner size="sm" /> : <CreditCard className="h-4 w-4 mr-2" />}
...
export default EditarContaPagar;
